...
export default async function ReadPost({ params }: PageProps) {
  const { id } = params; // Ekstraktovanje id iz params
  const postRef = collection(db, "posts");
  const q = query(postRef, where("blogID", "==", id));
  const snapshot = await getDocs(q);
  
  // Uzimanje prvog posta iz rezultata
  const post: PostType | undefined = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id }))[0] as PostType | undefined;
